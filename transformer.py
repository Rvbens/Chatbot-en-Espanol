...
def create_masks(input_seq, target_seq): #(bs,sl)
    # creates mask with 0s wherever there is padding in the input
    input_msk = (input_seq != PAD_token).unsqueeze(1) #(bs,1, sl)
...
# input_seq = input_variable.transpose(0,1) # (sl,bs) -> (bs,sl)
# target_seq = target_variable.transpose(0,1)

# src_mask, trg_mask = create_masks(input_seq, target_seq)
# src_mask.shape,trg_mask.shape, trg_mask[0]
...
def attention(q, k, v, d_k, mask=None, dropout=None):
    ...
    scores = torch.matmul(q, k.transpose(-2, -1)) /  math.sqrt(d_k) #(bs,N,sl_q,sl_k)
    if mask is not None:
